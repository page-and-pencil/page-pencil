...
## 1단계 — 원장님이 Supabase 대시보드에서 5분 (코드 변경 불필요) ※ 2026-07-11 보류
(시도했으나 대시보드 함수 배포 단계에서 보류하기로 함. 앱은 기존 방식 그대로 동작 중 —
아무것도 깨지지 않음. 재개 시: 아래 2번 코드 배포만 하면 됨. 함수 소스는 이미 시크릿 우선으로 준비됨)
